...
pub(crate) fn load_voices(asset: Res<AssetServer>, mut commands: Commands) {
    ...
    let mut add = |s: Species, vol: f32, files: &[&'static str]| {
        m.insert(s, VoiceSet { clips: files.iter().map(|f| asset.load(*f)).collect(), volume: vol });
    };
...
pub(crate) fn animal_voices(
    ...
    for (e, mut a, gt) in &mut q {
        ...
        a.voice_timer = rng_range(&mut a.rng, cfg.call_min, cfg.call_max);
